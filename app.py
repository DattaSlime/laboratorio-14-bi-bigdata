...
# Configuración de diseño de la página en la nube
...
def cargar_datos():
    try:
        # Intenta cargar el archivo que subiste al repositorio
        return pd.read_csv("dataset_personal.csv")
    except FileNotFoundError:
        # Respaldo automático por si falla la ruta en el servidor externo
        np.random.seed(125)
        n = 3125
        df_alt = pd.DataFrame({
...
ax3.set_xlabel("Índice de Estrés Corporal (Presión / Sueño)")
...
with col_h:
    ...
    st.markdown("""
    * **Hallazgo 1:** La tasa global de riesgo se sitúa en un **50.00%**, demostrando una división balanceada de la muestra derivada lógicamente mediante factores de riesgo ponderados.
    * **Hallazgo 2:** La población de **Adulto Mayor** (mayores de 60 años) muestra una incidencia de riesgo significativamente más alta, lo que valida empíricamente el peso asignado a la variable derivada `Riesgo_Edad`.
    * **Hallazgo 3:** Los diagramas estadísticos revelan contundentemente que los pacientes con diagnósticos en riesgo promedian menos de 6 horas de sueño diarias, impactando de forma directa en su presión arterial.
    """)

with col_r:
    st.subheader("🚀 Recomendaciones Organizacionales")
    st.markdown("""
    * **Recomendación 1:** Implementar de inmediato una Unidad de Alerta Preventiva Geriátrica para dar seguimiento clínico prioritario a pacientes mayores de 60 años.
    * **Recomendación 2:** Desarrollar programas institucionales enfocados en medicina cardiovascular e higiene del descanso para reducir los valores promedio del `Indice_Estres` en la población sana.
    * **Recomendación 3:** Integrar de manera informática los pesos del modelo de Machine Learning (**Random Forest**) directamente en el sistema de triaje del hospital para clasificar de forma automatizada al paciente durante el ingreso.
    """)